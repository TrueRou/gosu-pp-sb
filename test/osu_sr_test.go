...
func TestOsuPPDT(t *testing.T) {
	...
	osuFile, err := os.Open("1241370.osu")
	if err != nil {
		t.Fatal(err)
	}
	...
	beatMap.Difficulty.SetMods(difficulty.DoubleTime + difficulty.Relax + difficulty.Hidden)
	//beatMap.Difficulty.SetMods(difficulty.Relax)

	stars := osu.CalculateSingle(beatMap.HitObjects, beatMap.Difficulty)

	pp := &osu.PPv2{}
	//pp.PPv2x(stars, stars.MaxCombo, stars.ObjectCount, 0, 0, 0, beatMap.Difficulty)
	pp.PPv2x(stars, 2453, 2214, 8, 0, 1, beatMap.Difficulty)
	//math.Pow(pp.Results.Speed, 0.8)
	fmt.Println(pp.Results.Total)
