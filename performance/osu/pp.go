...
func (pp *PPv2) PPv2x(attribs Attributes, combo, n300, n100, n50, nmiss int, diff *difficulty.Difficulty) PPv2 {
	...
	var spdFactor float64 = 1.1
	if pp.diff.Mods.Active(difficulty.Relax) {
		spdFactor = 0.75
	}
	var aimFactor float64 = 1.1
	if pp.diff.Mods.Active(difficulty.Relax2) {
		aimFactor = 0.8
	}

	pp.Results.Total = math.Pow(
		math.Pow(pp.Results.Aim, aimFactor)+
			math.Pow(pp.Results.Speed, spdFactor)+
			math.Pow(pp.Results.Acc, 1.1)+
			math.Pow(pp.Results.Flashlight, 1.1),
		1.0/1.1) * finalMultiplier

	if pp.diff.Mods.Active(difficulty.Relax) {
		pp.Results.Total *= 1.2
	}
...
func (pp *PPv2) computeAccuracyValue() float64 {
	...
	var odTweaked float64 = pp.diff.ODReal
	if pp.diff.Mods.Active(difficulty.Relax) {
		odTweaked = math.Pow(odTweaked, 1.005)
	}
	accuracyValue := math.Pow(1.52163, odTweaked) * math.Pow(betterAccuracyPercentage, 24) * 2.83
